...
This code needs the following files in the same directory of the code:
    ...
    txt-pdftotext-fed/ - THIS CAN BE CHANGED WITH THE VARIABLE TEXT_DATA_DIR on line 224
        containing all .pkl files of pdftotext rips of collective agreements - created with DownloadData.py
    models/
...
def sentence_to_vec_idx(s2v_sentences: List[List[int]], counts, embedding_matrix, a: float=1e-3,from_persisted=False,persist=False,ver=''):
    ...
    if from_persisted:
        u = np.load("models/pca_component"+ver+".npy")
    else:
    ...
    if persist:
        np.save("models/pca_component"+ver, u)
...
if __name__ == "__main__":
    ...
    try:
        ...
        with open(filename,"r") as english_dictionary:
            ...
            pickle.dump(valid_words,open('models/english_words.pkl','wb'))
    ...
    try:
        ...
        pickle.dump(valid_words,open('models/french_words.pkl','wb'))
    ...
    pickle.dump(idx_to_filename,open('models/idx_to_filename.pkl','wb'))
    ...
    pickle.dump(idx_to_metadata,open('models/idx_to_metadata.pkl','wb'))
    ...
    pickle.dump(wvvocab_to_idx,open("models/wvvocab_to_idx.pkl",'wb'))
    del listvocab
    wvwords = [wvvocab_to_idx[word] for word in wvwords]
    counts = Counter(wvwords)
    del wvwords
    pickle.dump(counts,open("models/word_counts.pkl",'wb'))

    #allvocab: to test if word exist
    #allvocab_to_idx: transform word into it's idx number for stockage purposes
    #limite des mots dans les dictionnaires
    allvocab = set([word for passage in passages_by_words for word in passage if (word in english_words or word in french_words or word in wvvocab)])
    listvocab = list(allvocab)
    allvocab_to_idx = {listvocab[index]:index for index in range(len(listvocab))}
    pickle.dump(allvocab_to_idx,open("models/allvocab_to_idx.pkl",'wb'))
    ...
    pickle.dump(embedding_matrix,open("models/embedding_matrix.pkl",'wb'))
    ...
    pickle.dump(sentence_vectors,open("models/sentence_vectors.pkl",'wb'))

    #sentence_words: liste des word_idx de allvocab_to_idx dans la page correspondant à sentence_vectors
    ...
    dtyp = 'u4'
    if max(sentence_words) <= 65535: dtyp = 'u2'
    sentence_words = np.array(sentence_words,dtype=dtyp)
    sentence_words_idx = np.array(sentence_words_idx,dtype='u4')
    pickle.dump(sentence_words,open("models/sentence_words.pkl",'wb'))
    pickle.dump(sentence_words_idx,open("models/sentence_words_idx.pkl",'wb'))
