...
pklpath = 'F:\\CASA\\txt-pdftotext-fed\\'
execpath = 'C:\\Users\\nicolas.berube\\xpdf-tools-win-4.00\\bin64\\'
...
for icount,url in enumerate(urls):
    ...
    if coll.split('.')[0]+".pkl" in dirlist:
        ...
        for i in range(1,nopage):
            processlist.append(subprocess.Popen(execpath+'pdftotext -f %i -l %i -layout '%(i,i)+pklpath+coll+' '+pklpath+coll[:-4]+'%.4i.txt'%i))
        exit_codes = [pr.wait() for pr in processlist]
        #Lecture des données
        for i in range(1,nopage):
        ...
        processlist = []
        for i in range(1,nopage):
            processlist.append(subprocess.Popen('del '+pklpath+coll[:-4]+'%.4i.txt'%i,shell=True))
        exit_codes = [pr.wait() for pr in processlist]
